...
|--------|------------------------------|--------|
| [omniwitness](https://github.com/transparency-dev/witness) | A log configured for omniwitness can point the same `add-checkpoint` traffic at mosskeys-witness unmodified; our `0x04` line verifies against omniwitness-compatible verifiers. Config file is a (origin, vkey) list in the same spirit so operators can multi-home both witnesses on the same logs. | ⬜ live test (task 7) |
| [sigsum](https://git.glasklar.is/sigsum) | Our `0x04` cosignatures verify with sigsum's note verification. | ⬜ live test (task 7) |
